...
`acmed-issuer-acmesh` is a remote issuer plugin for the `acmed` gen2 stack.
It runs `acme.sh` behind a small HTTP API so `acmed` can delegate issuance to
...
- your environment already uses `acme.sh` DNS integrations
...
2. runs `acme.sh --issue ...`
3. runs `acme.sh --install-cert ...` into a per-order state directory
...
- `profile.plugin_name` is required for `/issue` (for example `dns_cf`)
- acme.sh binary is installed into the image during build
...
- `plugin_name` (required): acme.sh DNS plugin name, e.g. `dns_cf`
...
- `credential_env` (optional list): env var names copied into the `acme.sh` process
- `executable` (optional): explicit acme.sh binary path
...
Optional SDK source override at build time:
...
- `dependency_missing` for acme.sh:
  image/addon build issue, verify `/usr/local/bin/acme.sh` exists
