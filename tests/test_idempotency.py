...
def test_acmesh_plugin_caches_by_order_id(monkeypatch, tmp_path: Path):
    ...
    monkeypatch.setattr("acmed_issuer_acmesh.plugin.subprocess.run", fake_run)
    monkeypatch.setattr("acmed_issuer_acmesh.plugin.shutil.which", lambda _: "/usr/local/bin/acme.sh")
    ...
    req = IssueRequest(
        ...
        profile={"plugin_name": "dns_cf", "challenge_mode": "dns-01"},
    )
    r1 = plugin.issue(req)
    r2 = plugin.issue(req)
    ...
    assert calls["count"] == 2  # issue + install on first call only
